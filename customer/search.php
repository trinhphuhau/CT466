...
<body>
    ...
    <div class="main">
        <div class="container">
            <form action="search.php?s=" method="get">
                <input type="text" name="search" placeholder="What's are you looking for?" class="search-bar__input"  style="width: 100%" value="<?php echo $_GET["search"] ?>" autocomplete="off">
                <input type="submit" class="hide">
            </form>
        </div>
        <div class="container">
            ...
            ?>
            <h1 style="padding-bottom: 10px; border-bottom: 1px solid black">About <?php echo $total_records; ?> result(s)</h1>
            <div class="search-page flex">
                <?php
                    $product = $conn->query("SELECT * FROM sanpham WHERE ten LIKE '%$search%'");
                    while ($row_search = $product->fetch_assoc()) { ?>
                <div class="fashion-product">
                    <a href="../product.php?id=<?php echo $row_search["idsp"] ?>">
                        <img src="<?php echo $row_search["hinhanh"] ?>" alt="<?php echo $row_search["ten"] ?>" class="fashion-product-img">
                    </a>
                    <div class="fashion-product-info">
                        <a href="../product.php?id=<?php echo $row_search["idsp"] ?>">
                            <div class="fashion-product-name"><?php echo $row_search["ten"] ?></div>
                        </a>
                        <div class="fashion-product-price"><?php echo number_format($row_search["giaban"],0,",",".") ?>₫</div>
                    </div>
                </div>
                <?php    }?>
            </div>
