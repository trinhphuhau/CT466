<?php
    ...
    $fashion = "SELECT * FROM sanpham WHERE gioitinh IN (0,2)";
    if (isset($_GET["type"])) {
        $maloai = $_GET["type"];
        $check = $conn->query("SELECT maloai FROM loai WHERE maloai = '$maloai' AND gioitinh = 2 OR maloai = '$maloai' AND gioitinh = 0");
        if ($check->num_rows > 0) {
            $fashion = "SELECT * FROM sanpham WHERE maloai = '$maloai' AND gioitinh IN (0,2)";
        }
    }
...
<body>
        ...
        <div class="navigation">
            ...
            <div class="navigation-link" id="linknenenene">
                <ul class="menu">
                    ...
                    <li class="list-menu">
                        <div class="dropdown-menu" id="male-menu">
                            <a href="../male-fashion.php" class="list-title list-title-active">Men</a>
    ...
    <div class="main">
        <div class="container">
            <h1 class="nav-title">Men's Fashion</h1>
        </div>
        <div class="container">
                    ...
                    <div class="order-history-nav" id="ao">
                        <ul>
                            <?php
                                $ao = $conn->query("SELECT * FROM loai WHERE loai = 'Top' AND gioitinh IN (0, 2)");
                                while ($row_ao = $ao->fetch_assoc()) {?>
                                    <a href="../male-fashion.php?type=<?php echo $row_ao["maloai"] ?>" class="fashion-type-ao"><li <?php if (isset($_GET["type"])){
                                            if($row_ao["maloai"] == $_GET["type"])
                    ...
                    <div class="order-history-nav" id="quan">
                        <ul>
                            <?php
                                $quan = $conn->query("SELECT * FROM loai WHERE loai = 'Bottom' AND gioitinh IN (0, 2)");
                                while ($row_quan = $quan->fetch_assoc()) {?>
                                    <a href="../male-fashion.php?type=<?php echo $row_quan["maloai"] ?>" class="fashion-type-quan"><li <?php if (isset($_GET["type"])){
                                            if($row_quan["maloai"] == $_GET["type"])
                                                echo "class='fashion-active'";
                                        }
