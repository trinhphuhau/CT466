...
<body>
    ...
    <div class="main">
        <div class="container">
            <h1 class="nav-title">Order History</h1>
        </div>
        <div class="container">
            <div class="order-history-grid">
                <div class="order-history-col1">
                    <?php
                        ...
                        if(isset($_GET["tt"])) {
                            $tinhtrang = $_GET["tt"];
                            $donhang = $conn->query("SELECT * FROM donhang dh, donhangtinhtrang dhtt WHERE idkh = '$idkh' AND dh.matt = '$tinhtrang' AND dh.matt = dhtt.matt");
                            $history_tt = $conn->query("SELECT tinhtrang FROM donhangtinhtrang WHERE matt = '$tinhtrang'");
                            $row_history_tt = $history_tt->fetch_assoc();
                            $current_tt = $row_history_tt["tinhtrang"];
                        } else {
                            $donhang = $conn->query("SELECT * FROM donhang dh, donhangtinhtrang dhtt WHERE idkh = '$idkh' AND dh.matt = dhtt.matt");
                            $current_tt = "All";
                        }
                    ?>
                    <div class="tinhtrang between" id="history-tinhtrang" onclick="openTinhTrang()">
                        <h2><?php echo $current_tt ?></h2>
                        <span class="material-icons" id="tinhtrangIcon">expand_more</span>
                    </div>
                    <div class="order-history-nav" id="tinhtrang">
                        <ul>
                            <a href="../khachhang/ordered-history.php" class="tinhtrang-type"><li <?php if($tinhtrang == "") echo ' class="fashion-active"' ?>>All</li></a>
                            <?php
                                $tt = $conn->query("SELECT * FROM donhangtinhtrang ORDER BY stt");
                                while($row_tt = $tt->fetch_assoc()) { ?>
                            <a href="../khachhang/ordered-history.php?tt=<?php echo $row_tt["matt"] ?>" class="tinhtrang-type"><li <?php if($tinhtrang == $row_tt["matt"]) echo ' class="fashion-active"' ?>><?php echo $row_tt["tinhtrang"] ?></li></a>
                            <?php    }
                            ?>
                        </ul>
                    </div>
                ...
                <div class="order-history-col2" style="position: relative;">
                    <div class="order-history-details">
                        ...
                        if ($donhang->num_rows > 0) {
                            ...
                            ?>
                        <a href="../khachhang/ordered-details.php?madh=<?php echo $row_donhang["madh"] ?>" title="Xem chi tiết đơn hàng">
                            ...
                            <div class="ordered-info-last-row" style="padding: 10px 10px 0px 10px">
                                <div class="order-money" style="font-size: 20px; font-weight: 600;">
                                    <span>Total:</span>
                        ...
                        <div class="order-history-empty">
                            <h2>Hiện tại vẫn chưa có đơn hàng nào</h2>
                            <img src="../img/empty.png" alt="Trống rỗng">
                            <h2><a href="../index.php">Nhấn vào đây để về trang chủ</a></h2>
                        </div>
                        <?php } ?>
                        </div>
                    </div>
...
<script>
    var openTinhTrangClick = 0;
    var tinhTrangType = document.getElementsByClassName("tinhtrang-type");
    var ttContainer_height = tinhTrangType.length*42;
    var watchMediaTinhTrang = window.matchMedia("(max-width: 1024px)");
    
    function watchTinhTrang(watchMedia) {
        if (!watchMediaTinhTrang.matches) {
            document.getElementById("tinhtrang").style.height = ttContainer_height+"px";
            document.getElementById("tinhtrangIcon").innerHTML = "expand_less";
        } else {
            if (openTinhTrangClick%2 == 0) {
                document.getElementById("tinhtrang").style.height = ttContainer_height+"px";
                document.getElementById("tinhtrangIcon").innerHTML = "expand_less";
            }
            if (openTinhTrangClick%2 == 0) {
                document.getElementById("tinhtrang").style.height = "0";
                document.getElementById("tinhtrangIcon").innerHTML = "expand_more";
            }
        }
    }

    watchTinhTrang(watchMediaTinhTrang);
    watchMediaTinhTrang.addListener(watchTinhTrang);

    function openTinhTrang() {
        openTinhTrangClick++;
        if (openTinhTrangClick%2 == 1) {
            document.getElementById("tinhtrang").style.height = ttContainer_height+"px";
            document.getElementById("tinhtrang").style.marginTop = "10px";
            document.getElementById("tinhtrangIcon").innerHTML = "expand_less";
        } else {
            document.getElementById("tinhtrang").style.height = "0";
            document.getElementById("tinhtrang").style.marginTop = "0px";
            document.getElementById("tinhtrangIcon").innerHTML = "expand_more";
        }
    }
